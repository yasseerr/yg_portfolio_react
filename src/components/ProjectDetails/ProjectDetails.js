...
function ImageContainer(props) {
  let im = require("../../data/"+props.src).default;
  return (
...
const ProjectDetails = (props) => {
  ...
  const tick = ()=> imageIndex<(project.images.length-1)?setImageIndex(imageIndex+1):"";
  ...
  project = project?project:{name: "",description: "",images:[],link: ""}
  console.log(project)
  return(
    <div className="ProjectDetails" style={project_style}>
      ...
      <div className="gallery">
        <div style={{height: "100%",width: "100%", transform: image_translate, transition: "1s ease-in-out"}}>
          {project.images.map((i)=>(<ImageContainer src={i}/>))}
        </div>
